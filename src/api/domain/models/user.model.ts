...
// Old code of allUser or search user 1
/*
export const getAllUsersLogic = async(
    ...
    try {
        ...
        // Exclude the logged-in user if `loggedInUserId` is provided
        if (loggedInUserId) {
            pipeline.push({
                $match: { _id: { $ne: new mongoose.Types.ObjectId(loggedInUserId) } },
            });
        }
        ...
        if (username) {
            pipeline.push({
                $match: {
                    $or: [
                        { userName: { $regex: username, $options: 'i' } }, // Case-insensitive search on userName
                        { name: { $regex: username, $options: 'i' } },    // Case-insensitive search on name
                    ],
                },
            });
        }

        // Pagination: Skip and limit users
        ...
        if (loggedInUserId) {
            countQuery._id = { $ne: new mongoose.Types.ObjectId(loggedInUserId) };
        }
        if (username) {
            countQuery.$or = [
        ...
        const totalUsers = await userSchema.countDocuments(countQuery);
        const response = {
            users,
...
export const getAllUsersLogic123 = async (
    loggedInUserId: string | undefined,
    pagination: { page: number; limit: number },
    searchQuery: string | undefined,
    contactNumbers: string | undefined,
    callback: (error: any, result: any) => void
) => {
    try {
        const { page, limit } = pagination;
        ...
        const currentUser = await userSchema.findById(loggedInUserId).select("contacts");
        if (!currentUser) {
            return callback(
                {
                    status: 404,
        ...
        // Normalize contacts (remove spaces, take last 10 digits)
        ...
        try {
            contactArray = contactNumbers ? JSON.parse(contactNumbers) : [];
        } catch (e) {
            contactArray = [];
        }

        try {
            searchArray = searchQuery ? JSON.parse(searchQuery) : [];
        } catch (e) {
            searchArray = [];
        }
        ...
        if (Array.isArray(contactArray) && contactArray.length > 0) {
            // If `contactNumbers` is provided, match exact contacts only
            pipeline.push({
                $match: {
                    phone: { $in: contactArray },
                },
            });
        } else if (Array.isArray(searchArray) && searchArray.length > 0) {
            // If `searchQuery` is an array of phone numbers, match exact contacts
            pipeline.push({
                $match: {
                    phone: { $in: searchArray },
                },
            });
        } else {
            // General user search with regex matching
            pipeline.push({
                $match: {
                    $or: [
                        {
        ...
        const totalUsers = await userSchema.countDocuments({
            ...
            $or: [
                ...
                { allowUserName: true, userName: { $regex: escapedSearchQuery, $options: "i" } },
            ],
        });
...
// original
export const getAllUsersLogic1 = async (
    loggedInUserId: string | undefined,
    pagination: { page: number; limit: number },
    searchQuery: string | undefined,
    contactNumbers: string | undefined,
    ...
    try {
        ...
        const chatParticipants = await chatSchema.aggregate([
            {$match:{participants: new mongoose.Types.ObjectId(loggedInUserId)} },
            {
                $lookup: {
        ...
        const pipeline: any[] = [
            ...
            {
                $addFields: {
                    isContact: { $cond: [{ $in: ["$phone", normalizedContacts] }, 1, 0] },
                    isChatParticipant: {$cond: [{$in:["$_id", chatParticipants] }, 1, 0] },
                },
            },
        ];
        ...
        if (Array.isArray(contactArray) && contactArray.length > 0) {
            ...
            pipeline.push({
                $match: searchQuery
                    ...
                    : {
                          $or: [
                              { phone: { $in: normalizedContacts }, profilePrivacy: "private" },
            ...
            //         ? {
            //               $or: [
            //                   { profilePrivacy: "public" }, // ✅ Always allow public users
            //                   { phone: { $in: normalizedContacts }, profilePrivacy: "private" }, // ✅ Allow private users only if they are in contacts
            //               ],
            //               // @ts-ignore
            //               $or: [
            //                   { userName: { $regex: escapedSearchQuery, $options: "i" } },
            ...
            //         : {
            //               $or: [
            //                   { phone: { $in: normalizedContacts }, profilePrivacy: "private" }, // ✅ Show private contacts in list
            //                   { _id: { $in: chatParticipants } }, // ✅ Show chat participants
            //               ],
            //           },
            // });
        }

        // Sort by contacts first, then by name
        pipeline.push({ $sort: { isContact: -1, name: 1 } });
        ...
        const totalUsers = await userSchema.countDocuments({
            ...
            $or: [
                { phone: { $in: normalizedContacts }, profilePrivacy: "private" },
                {_id:{$in: chatParticipants} }, // Include chat participants
                searchQuery
                    ? {
                          profilePrivacy: "public",
                        //   allowUserName: true,
                            ...
                            // allowEmail: true,
                          $or: [
                              { userName: { $regex: escapedSearchQuery, $options: "i" }, /* allowUserName: true */},
                              { name: { $regex: escapedSearchQuery, $options: "i" } },
                              { phone: { $regex: escapedSearchQuery, $options: "i" }, /* allowPhone: true */},
                              { email: { $regex: escapedSearchQuery, $options: "i" }, /* allowEmail: true */},
                          ],
                        //   phone: { $nin: normalizedContacts }, // Exclude contacts from search results
                      }
                    : null,
            ].filter(Boolean),
        });

        return callback(null, { users, totalUsers });
    } catch (error) {
        return callback(
            {
                status: 500,
...
// New try 31-03-25 ( working query contacts list + searching public user + searching contact user)
export const getAllUsersLogic2 = async (
    loggedInUserId: string | undefined,
    pagination: { page: number; limit: number },
    searchQuery: string | undefined,
    callback: (error: any, result: any) => void
    ...
    try {
        ...
        const chatParticipants = await chatSchema.find({ participants: loggedInUserId }).distinct("participants");
        const chatUserIds = chatParticipants
            .map(id => id.toString())
        ...
        let matchCondition: any = {
            $or: [
                ...
                { _id: { $in: chatUserIds } } // Users with whom the user has chatted
            ],
            isVerified: true,
            isProfileSetUp: true
        };
